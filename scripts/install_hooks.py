...
def hooks_match(existing: dict, new_hook: dict) -> bool:
    ...
    if existing.get("type") != new_hook.get("type"):
        return False
    ec = existing.get("command")
    nc = new_hook.get("command")
    if ec and nc and ec == nc:
        return True
    ep = existing.get("prompt")
    np_ = new_hook.get("prompt")
    if ep and np_ and ep == np_:
        return True
    return False
...
def uninstall(args):
    ...
    for event_name in list(settings_hooks.keys()):
        ...
        for entry in entries:
            ...
            for h in hooks_list:
                cmd = h.get("command", "")
                if _HOOK_MARKER.lower() in cmd.lower():
                    is_pet = True
                    break
            if not is_pet:
